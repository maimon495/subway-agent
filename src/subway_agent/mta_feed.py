...
def get_trip_duration(origin_id: str, dest_id: str, trip_id: str) -> Optional[int]:
    """Get the real-time duration between two stations for a specific trip.

    Args:
        origin_id: The origin station ID
        dest_id: The destination station ID
        trip_id: The trip ID from a TrainArrival

    Returns:
        Duration in minutes, or None if not available
    """
    origin_station = STATIONS.get(origin_id)
    dest_station = STATIONS.get(dest_id)

    if not origin_station or not dest_station:
        return None

    # Find which feed this trip is on by checking the line prefix in trip_id
    feed_url = None
    for line, url in LINE_TO_FEED.items():
        if trip_id.startswith(line) or f"_{line}_" in trip_id:
            feed_url = url
            break

    # If we can't determine from trip_id, try all feeds the origin station uses
    if not feed_url:
        for line in origin_station.lines:
            if line in LINE_TO_FEED:
                feed_url = LINE_TO_FEED[line]
                break

    if not feed_url:
        return None

    feed = mta_feed._fetch_feed(feed_url)
    if not feed:
        return None

    # Find the trip and get arrival times at both stations
    for entity in feed.entity:
        if not entity.HasField("trip_update"):
            continue
        ...
        for stop_time_update in trip_update.stop_time_update:
            gtfs_stop_id = stop_time_update.stop_id[:-1] if stop_time_update.stop_id[-1] in ("N", "S") else stop_time_update.stop_id

            if gtfs_stop_id == origin_station.gtfs_stop_id:
                if stop_time_update.HasField("arrival"):
                    origin_time = stop_time_update.arrival.time
                elif stop_time_update.HasField("departure"):
                    origin_time = stop_time_update.departure.time

            if gtfs_stop_id == dest_station.gtfs_stop_id:
                if stop_time_update.HasField("arrival"):
                    dest_time = stop_time_update.arrival.time
                elif stop_time_update.HasField("departure"):
                    dest_time = stop_time_update.departure.time

        if origin_time and dest_time:
            return int((dest_time - origin_time) / 60)
